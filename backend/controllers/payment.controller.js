...
export const createCheckoutSession = async (req,res) => {
    try{
        ...
        const lineItems = products.map(product => {
            const amount = Math.round(Number(product.price) * 100) // Stripe wants you to send in the format of cents
        ...
        if(couponCode){
            ...
            if(coupon){
                totalAmount -= Math.round(totalAmount * coupon.discountPercantage / 100)
            }
        }
...
async function createStripeCoupon(discountPercantage){
    const coupon = await stripe.coupons.create({
        percent_off: discountPercantage,
        duration: "once",
    })
    return coupon.id
}
